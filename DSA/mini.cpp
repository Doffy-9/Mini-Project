...
public:
    ...
    double calculateBill(const string& type, time_t inTime) {
        ...
        double rate = 0;
        if (type == "bike" || type == "BIKE") rate = 10;
        else if (type == "car" || type == "CAR") rate = 20;
        else if (type == "bus" || type == "BUS") rate = 30;
        return rate * (hours > 0 ? hours : 1);
    }
...
int main() {
    ...
    parking.loadData(); // ✅ load after taking user input
